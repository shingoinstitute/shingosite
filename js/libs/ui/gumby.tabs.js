...
!function($) {
	...
	function Tabs($el) {
		...
		// !!!! the if else clause is do to an error experienced on mobile !!!!
		// !!!! where the tab wouldn't change dymanically. However changing!!!!
		// !!!! Gumby.js to set Gumby.click = 'tap' for mobile breaks the  !!!!
		// !!!! ability to click the tab and have it change.               !!!!
		this.$el.on('gumby.set', function(e, index) {
			if(Modernizr.touch){
				Gumby.click = 'tap';
				Gumby.debug('Set event triggered', scope.$el);
				scope.set(e, index);
				Gumby.click = 'click tap';
			}
			else {
				Gumby.debug('Set event triggered', scope.$el);
				scope.set(e, index);
